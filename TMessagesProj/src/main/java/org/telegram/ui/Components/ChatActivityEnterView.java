                    ...
                    if (recordCircle.isSendButtonVisible()) {
                        if (!hasRecordVideo || calledRecordRunnable) {
                            ...
                            if (hasRecordVideo && isInVideoMode()) {
                                ...
                                delegate.needStartRecordVideo(OctoConfig.INSTANCE.promptBeforeSendingVideoMessages.getValue() ? 3 : 1, true, 0, 0, voiceOnce ? 0x7FFFFFFF : 0, effectId, 0);
...
//            }
    ...
    public void setInputBoxStyle(InputBoxStyle style) {
        if (style == null) {
            style = InputBoxStyle.LIQUID_GLASS;
        }
        if (inputBoxStyle != style) {
            inputBoxStyle = style;
            invalidate();
        }
    }

    public InputBoxStyle getInputBoxStyle() {
        return inputBoxStyle;
    }

    private Drawable getComposeShadowDrawableForStyle() {
        if (inputBoxStyle == InputBoxStyle.MATERIAL3 && Theme.chat_composeShadowRoundDrawable != null) {
            return Theme.chat_composeShadowRoundDrawable;
        }
        return Theme.chat_composeShadowDrawable;
    }

    private InputBoxStyle getConfigInputBoxStyle() {
        InputBoxStyle[] styles = InputBoxStyle.values();
        int value = OctoConfig.INSTANCE.inputBoxStyle.getValue();
    ...
    private InputBoxStyle inputBoxStyle = InputBoxStyle.LIQUID_GLASS;

    Paint backgroundPaint = new Paint();
    private float composeShadowAlpha = 1f;
    private Rect blurBounds = new Rect();
    private final Path md3BackgroundPath = new Path();
    private final RectF md3BackgroundRect = new RectF();
    private final float[] md3CornerRadii = new float[8];
    @Override
    protected void onDraw(Canvas canvas) {
    ...
    public void drawBackground(Canvas canvas, boolean withComposeShadowDrawable) {
        ...
        final Drawable composeShadowDrawable = getComposeShadowDrawableForStyle();
        final int composeShadowHeight = composeShadowDrawable != null ? composeShadowDrawable.getIntrinsicHeight() : 0;
        int top = animatedTop;
        top += composeShadowHeight * (1f - composeShadowAlpha);
        if (topView != null && topView.getVisibility() == View.VISIBLE) {
            top += (1f - getTopViewEnterProgress()) * topView.getLayoutParams().height;
        }
        int bottom = top + composeShadowHeight;

        if (withComposeShadowDrawable && composeShadowDrawable != null) {
            composeShadowDrawable.setAlpha((int) (composeShadowAlpha * 0xFF));
            composeShadowDrawable.setBounds(0, top, getMeasuredWidth(), bottom);
            composeShadowDrawable.draw(canvas);
        }

        final int backgroundTop = bottom;
        final int backgroundBottom = getHeight();
        final Paint fallbackPaint = getThemedPaint(Theme.key_paint_chatComposeBackground);
        final int backgroundColor = getThemedColor(Theme.key_chat_messagePanelBackground);
        backgroundPaint.setColor(backgroundColor);

        switch (inputBoxStyle) {
            case LIQUID_GLASS:
                if (allowBlur && SharedConfig.chatBlurEnabled() && sizeNotifierLayout != null) {
                    blurBounds.set(0, backgroundTop, getWidth(), backgroundBottom);
                    sizeNotifierLayout.drawBlurRect(canvas, getTop(), blurBounds, backgroundPaint, false);
                } else {
                    canvas.drawRect(0, backgroundTop, getWidth(), backgroundBottom, fallbackPaint);
                }
                break;
            case STOCK_SOLID:
                canvas.drawRect(0, backgroundTop, getWidth(), backgroundBottom, backgroundPaint);
                break;
            case MATERIAL3:
                final float radius = dp(12);
                md3BackgroundRect.set(0, backgroundTop, getWidth(), backgroundBottom);
                md3CornerRadii[0] = radius;
                md3CornerRadii[1] = radius;
                md3CornerRadii[2] = radius;
                md3CornerRadii[3] = radius;
                md3CornerRadii[4] = 0;
                md3CornerRadii[5] = 0;
                md3CornerRadii[6] = 0;
                md3CornerRadii[7] = 0;
                md3BackgroundPath.reset();
                md3BackgroundPath.addRoundRect(md3BackgroundRect, md3CornerRadii, Path.Direction.CW);
                canvas.drawPath(md3BackgroundPath, backgroundPaint);
                break;
        }
    }
...
//        }
        ...
        if (translateButtonValue) {
            ...
            subItem.setTextAndIcon(LocaleController.formatString(R.string.TranslateToButton, translatedLanguageName), R.drawable.msg_translate, null);
...
//                            }
    ...
    private void createEmojiView() {
        ...
        emojiView.setDelegate(new EmojiView.EmojiViewDelegate() {
            ...
            public void onGifSelected(View view, Object gif, String query, Object parent, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
                ...
                onGifSelectedFinal(view, gif, query, parent, notify, scheduleDate, scheduleRepeatPeriod);
            }
    ...
    public void onStickerSelected(TLRPC.Document sticker, String query, Object parent, MessageObject.SendAnimationData sendAnimationData, boolean clearsInputField, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
        if (OctoConfig.INSTANCE.promptBeforeSendingStickers.getValue()) {
            PopupPromptUtils.createAlertDialog(parentActivity, PromptBeforeSendMedia.STICKERS, () -> onStickerSelectedFinal(sticker, query, parent, sendAnimationData, clearsInputField, notify, scheduleDate, scheduleRepeatPeriod));
            return;
        }

        onStickerSelectedFinal(sticker, query, parent, sendAnimationData, clearsInputField, notify, scheduleDate, scheduleRepeatPeriod);
    }

    public void onStickerSelectedFinal(TLRPC.Document sticker, String query, Object parent, MessageObject.SendAnimationData sendAnimationData, boolean clearsInputField, boolean notify, int scheduleDate, int scheduleRepeatPeriod) {
        if (replyingQuote != null && parentFragment != null && replyingQuote.outdated) {
            parentFragment.showQuoteMessageUpdate();
            return;
        }
